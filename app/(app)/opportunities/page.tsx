...
// dashboard only shows a top-4 preview). Merged into /opportunities
// (formerly its own /market-intelligence route) since it's the same data
// as /opportunities/[id] — this is the list, that's the detail page. Cards
// link into that existing detail page rather than duplicating its layout.
...
export default async function MarketIntelligencePage({
  ...
  return (
    <AppShell activePath="/opportunities">
      <h1 className="text-headline-lg text-on-surface mt-6 mb-1">Opportunities</h1>
      <p className="text-body-md text-on-surface-variant mb-4">
        {visible.length} of {opportunities.length} matched jobs.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <aside className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col gap-6 h-fit">
          <div className="flex justify-between items-center">
            ...
            <Link href="/opportunities" className="text-secondary text-label-md hover:underline">
              Reset
            </Link>
          </div>
