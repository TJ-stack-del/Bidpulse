...
// submissions, not just one contractor's own. Admin-only, gated by RLS
// (a client account querying this table only ever sees their own row,
// per schema.sql's "clients manage their own submissions" policy).
...
export default async function AdminInboxPage() {
  ...
  if (!user) {
    return (
      <AppShell activePath="/admin/inbox">
        <p className="text-body-md text-on-surface-variant mt-6">Please log in.</p>
      </AppShell>
    );
  }
