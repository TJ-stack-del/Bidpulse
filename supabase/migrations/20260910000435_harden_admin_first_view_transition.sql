-- Draft submissions share the default `submitted` stage with finalized
-- submissions. Prevent an admin preview of a draft from advancing workflow or
-- notifying the client, and preserve the first-view timestamp if a concurrent
-- manual transition reaches `in_review` first.
...
begin
  ...
  -- If a manual submitted -> in_review transition won the race, the page
  -- view is still real and should be recorded without duplicating the stage
  -- audit event or notification.
